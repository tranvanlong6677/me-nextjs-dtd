...
function AlertDialogDeleteTable({
  ...
  const handleDeleteTable = async (value: TableItem | null) => {
    if (!value) return
    try {
      const result = await deleteTable(value.number)
      toast({
    ...
    >
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={() => handleDeleteTable(tableDelete)}>Continue</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
...
export default function TableTable() {
  ...
  const { data: listTable } = useGetListTableQuery()
  const data: any[] = listTable?.payload.data ?? []
