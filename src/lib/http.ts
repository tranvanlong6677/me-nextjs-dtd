...
const request = async <Response>(
  ...
  if (isClient) {
    ...
    if (['api/auth/login', 'api/guest/auth/login', 'api/auth/token'].includes(normalizeUrl)) {
      const { accessToken, refreshToken } = (payload as LoginResType | GuestLoginResType).data
