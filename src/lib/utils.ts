...
export const getRoleFromLocalStorage = () => (isBrowser ? localStorage.getItem('role') : null)

export const setRoleToLocalStorage = (value: string) => isBrowser && localStorage.setItem('role', value)
...
export const getRoleFromClient = (accessToken: string) => {
...
export const wrapServerApi = async <T>(callback: () => Promise<T>) => {
  let result = null
  try {
